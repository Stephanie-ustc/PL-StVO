...
plSVO::plSVO(string configFile){
    ...
    maxStDist       = config.read_float("PointTracking","maxStDist",0.f);
    maxF2FDisp      = config.read_float("PointTracking","maxF2FDisp",0.04f);
    nFeatures       = config.read_float("PointTracking","nFeatures",500);
    ...
    // Line Tracking Parameters
    scale           = config.read_uint64_t("LineTracking","scale",1);
    nOctaves        = config.read_uint64_t("LineTracking","nOctaves",1);
    matchTh         = config.read_uint64_t("LineTracking","matchTh",40);
    matchDist       = config.read_uint64_t("LineTracking","matchDist",10);
    dispMax         = config.read_float("LineTracking","dispMax",1000.f);
    alphaMax        = config.read_float("LineTracking","alphaMax",10.f);
    horizTh         = config.read_float("LineTracking","horizTh",0.5f);
    flowTh          = config.read_float("LineTracking","flowTh",1000.f);
    alphaTh         = config.read_float("LineTracking","alphaTh",10.f);
    matchPr         = config.read_float("LineTracking","matchPr",0.6f);

    // Flags
    parallel        = config.read_bool("SVO","parallel",false);
    points          = config.read_bool("SVO","points",true);
    lines           = config.read_bool("SVO","lines",true);
    uncertainty     = config.read_bool("SVO","uncertainty",true);
